...
const UINT NUM_ELEMENTS = 1280 * 720;
...
int __cdecl main()
{
    ...
    FILE* fi = fopen("input.raw", "rb");
    for ( int i = 0; i < NUM_ELEMENTS; ++i ) 
    {
        unsigned char buffer[4];
        fread(buffer, 4, 1, fi);
        g_vBuf0[i].b = buffer[0];
        g_vBuf0[i].g = buffer[1];
        g_vBuf0[i].r = buffer[2];
        g_vBuf0[i].a = 0;
 ...
 //   CreateStructuredBuffer( g_pDevice, sizeof(BufType1), NUM_ELEMENTS, nullptr, &g_pBuf1);
    CreateRawBuffer(g_pDevice, NUM_ELEMENTS * sizeof(BufType1), nullptr, &g_pBuf1);
...
//    CreateStructuredBuffer( g_pDevice, sizeof(BufType), NUM_ELEMENTS, nullptr, &g_pBufResult );
    ...
    CreateBufferUAV(g_pDevice, g_pBuf1, &g_pBufResultUAV);
    ...
    {
        ID3D11Buffer* debugbuf = CreateAndCopyToDebugBuf( g_pDevice, g_pContext, g_pBuf1);
        ...
        p = (BufType1*)MappedResource.pData;

        // Verify that if Compute Shader has done right
        printf( "Verifying against CPU result..." );

        FILE* f = fopen("dump_444.raw", "wb");
        fwrite(p, NUM_ELEMENTS * 3, 1, f);
...
{    
    ...
    *ppContextOut = nullptr;
    ...
    if ( !bForceRef )
    {
        hr = D3D11CreateDevice(dxgiAdapter,                        // Use default graphics card
...
HRESULT CreateStructuredBuffer(ID3D11Device* pDevice, UINT uElementSize, UINT uCount, void* pInitData, ID3D11Buffer** ppBufOut)
{
    *ppBufOut = nullptr;
    ...
    if (pInitData)
    {
        ...
        unsigned int hr = pDevice->CreateBuffer(&desc, nullptr, ppBufOut);
        return hr;
                      ...
                      UINT X, UINT Y, UINT Z )
{
    ...
    pd3dImmediateContext->Dispatch( (X / 4) / 16, Y, Z);
